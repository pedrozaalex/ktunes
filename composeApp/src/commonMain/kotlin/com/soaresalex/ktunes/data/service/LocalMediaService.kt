...
class LocalMediaService(
	...
	override suspend fun getTracks(): List<Track> {
		...
		return files.mapNotNull { file ->
			runCatching {
				...
				Logger.e(
					"Error extracting metadata from file: ${file.name}", it
				)
				null
			}
		}
	...
	private fun extractLocalFileMetadata(file: File): Track? {
		return try {
			...
			Logger.e(
				"Error extracting metadata from file: ${file.name}", e
			)
			null
		}
	}
		...
		private fun createTempArtworkFile(artwork: Artwork): String? {
			return try { // Determine file extension based on MIME type
				...
				val tempFile = File.createTempFile(
					"album_art_", extension, File(System.getProperty("java.io.tmpdir"))
				)
